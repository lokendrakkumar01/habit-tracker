...
const HabitModal = ({ open, onClose, initialData, onSave }) => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          className="relative w-full max-w-lg overflow-y-auto max-h-[90vh] rounded-3xl border p-6 sm:p-8 shadow-2xl space-y-6"
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-4">
            ...
            <button
              onClick={onClose}
              className="rounded-xl p-2 hover:bg-white/5 transition-colors border border-white/5"
            >
              <FiX size={16} />
            </button>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border px-4 py-3 outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all"
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full rounded-xl border px-4 py-3 outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 resize-none transition-all"
                style={{ borderColor: 'var(--border-default)', background: 'var(--bg-card)', color: 'var(--text-primary)' }}
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-xl border px-3 py-3 outline-none focus:border-violet-500 transition-all"
...
const HabitCard = ({ habit, tab, onEdit, onDelete, onArchive, onRestore, onComplete, onClick }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative cursor-pointer overflow-hidden rounded-2xl border p-5 backdrop-blur-md shadow-lg transition-all hover:shadow-indigo-500/5"
    ...
    >
      ...
      <div className="flex items-center gap-3.5 mt-2">
        <span className="text-4xl leading-none w-12 h-12 flex items-center justify-center rounded-xl border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-default)' }}>{habit.icon ?? '🎯'}</span>
        <div className="min-w-0 flex-1">
          <p className="truncate font-bold text-base tracking-tight leading-snug" style={{ color: 'var(--text-primary)' }}>{habit.title}</p>
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            <span className="rounded-lg px-2 py-0.5 text-[10px] font-semibold border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-subtle)', color: 'var(--text-secondary)' }}>
              {catLabel}
            </span>
...
const HabitsPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((h) => {
      const matchSearch =
        !search || (h.title && h.title.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="min-h-screen px-4 py-8 sm:px-8 space-y-6" style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <motion.div
          className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4 border-b"
        ...
        >
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
        ...
        >
          ...
          <div className="relative flex-1 min-w-[200px]">
            <span className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-muted)' }}><FiSearch size={16} /></span>
            <input
              type="text"
              placeholder="Search habits by name..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full rounded-xl border py-2.5 pl-10 pr-4 outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all text-sm"
              style={{ borderColor: 'var(--border-default)', background: 'var(--bg-card)', color: 'var(--text-primary)' }}
            />
          </div>
        ...
        >
          {['active', 'done', 'archived'].map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className="relative rounded-xl px-5 py-2 text-sm font-semibold capitalize transition-all cursor-pointer"
              style={{ color: tab === t ? 'var(--text-primary)' : 'var(--text-secondary)' }}
            >
              {tab === t && (
                <motion.div
