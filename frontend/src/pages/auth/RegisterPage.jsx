...
export default function RegisterPage() {
  ...
  const fieldStyle = (name) => ({
    ...
    boxShadow: focused === name ? '0 0 0 3px rgba(139,92,246,0.12)' : 'none',
  });
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-4">
        ...
        <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.5)', lineHeight: '1.6' }}>
          {isEmailVerify ? (
            <>We sent a verification link to <span style={{ color: '#a78bfa' }}>{form.email}</span>.<br />Click it to activate your account.</>
          ) : (
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="mb-6 text-center">
        ...
        <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>Start building better habits today</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <div>
          ...
          <div className="relative">
            <FiUser size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: focused === 'name' ? '#a78bfa' : 'rgba(255,255,255,0.3)' }} />
            <input
              name="name" type="text" placeholder="Lokendra Kumar" value={form.name}
        ...
        <div>
          ...
          <div className="relative">
            <FiMail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: focused === 'email' ? '#a78bfa' : 'rgba(255,255,255,0.3)' }} />
            <input
        ...
        <div>
          ...
          <div className="relative">
            <FiLock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: focused === 'password' ? '#a78bfa' : 'rgba(255,255,255,0.3)' }} />
            <input
              name="password" type={showPwd ? 'text' : 'password'} placeholder="Min. 6 characters" value={form.password}
          ...
          <AnimatePresence>
            {form.password && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-2 px-1">
                ...
                <p className="text-xs font-medium" style={{ color: strength ? strengthColor[strength] : 'transparent' }}>
        ...
        <div>
          ...
          <div className="relative">
            <FiLock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: focused === 'confirm' ? '#a78bfa' : 'rgba(255,255,255,0.3)' }} />
            <input
              name="confirm" type={showConf ? 'text' : 'password'} placeholder="Repeat your password" value={form.confirm}
              onChange={handleChange} onFocus={() => setFocused('confirm')} onBlur={() => setFocused('')}
              autoComplete="new-password"
              className="w-full rounded-xl py-3 pl-10 pr-11 text-sm text-white outline-none transition-all"
              style={{
                ...fieldStyle('confirm'),
                border: form.confirm && form.confirm !== form.password ? '1px solid rgba(239,68,68,0.7)' : fieldStyle('confirm').border,
              }}
            />
        ...
        <label className="flex items-start gap-3 cursor-pointer select-none pt-1">
          <button type="button" onClick={() => setTerms((v) => !v)}
            ...
            style={{
              background: terms ? '#7c3aed' : 'rgba(255,255,255,0.05)',
              border: terms ? '1px solid #7c3aed' : '1px solid rgba(255,255,255,0.2)',
            }}>
          ...
          <span className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.4)' }}>
            I agree to the{' '}
            <span style={{ color: '#a78bfa' }} className="cursor-pointer hover:underline">Terms of Service</span>
            {' '}and{' '}
            <span style={{ color: '#a78bfa' }} className="cursor-pointer hover:underline">Privacy Policy</span>
          </span>
        </label>
